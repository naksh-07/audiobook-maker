#!/usr/bin/env python3
"""
Produce Chapter 3: The Voice of Reason 2 (२: तर्क की आवाज़)
...
SCRIPT_PATH = PROJECT_DIR / "scripts" / "chapter_003_hi_script.json"
SOUNDSCAPE_PLAN = PROJECT_DIR / "soundscapes" / "chapter_003_hi_soundscape.json"
...
def main():
    ...
    logger.info("=" * 70)
    logger.info("🌿 PRODUCING CHAPTER 3: THE VOICE OF REASON 2 (तर्क की आवाज़ २)")
    logger.info("   Setting: Temple of Melitele Sanctuary, Mother Nenneke & Iola")
    ...
    # 2. Synthesize Chapter 3 Audio Segments
    logger.info(f"\n[*] Starting speech synthesis for Chapter 3...")
    audio_files = dispatcher.synthesize_chapter_script(SCRIPT_PATH, chapter_num=3)
    ...
    vocal_m4a = MASTERED_DIR / "chapter_003_mastered.m4a"
            ...
            "scenes": [
                {
                    ...
                    "mood": "peaceful",
                    "ambient_volume": 0.28,
                    "description": "Temple of Melitele sanctuary"
                }
            ],
            "sfx_cues": []
        }

    bgm_raw = SOUNDSCAPES_DIR / "chapter_003_bgm.wav"

    seg_durations = {}
    for seg in script_data:
        s_idx = seg.get("index", 1)
        matches = sorted(AUDIO_CHUNKS_DIR.glob(f"c003_s{s_idx:04d}_*.wav"))
        if matches:
    ...
    cinematic_m4a = MASTERED_DIR / "chapter_003_cinematic.m4a"
    apply_dynamic_sidechain_ducking(
    ...
    logger.info("=" * 70)
    logger.info("🎉 CHAPTER 3 PRODUCTION COMPLETE!")
