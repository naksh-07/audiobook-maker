#!/usr/bin/env python3
"""
Produce Chapter 4: A Grain of Truth (सच्चाई का एक कण - Nivellen & Vereena the Bruxa)
...
SCRIPT_PATH = PROJECT_DIR / "scripts" / "chapter_004_hi_script.json"
SOUNDSCAPE_PLAN = PROJECT_DIR / "soundscapes" / "chapter_004_hi_soundscape.json"
...
def main():
    ...
    logger.info("=" * 70)
    logger.info("🌹 PRODUCING CHAPTER 4: A GRAIN OF TRUTH (सच्चाई का एक कण)")
    logger.info("   Setting: Cursed Forest Manor, Nivellen & Vereena the Bruxa")
    ...
    dispatcher = TTSDispatcher(
        ...
        max_workers=1,  # Strict single-thread sequential pacing (100% human studio cadence)
    )

    # 2. Synthesize Chapter 4 Audio Segments
    logger.info(f"\n[*] Starting speech synthesis for Chapter 4...")
    audio_files = dispatcher.synthesize_chapter_script(SCRIPT_PATH, chapter_num=4)
    ...
    vocal_m4a = MASTERED_DIR / "chapter_004_mastered.m4a"
            ...
            "scenes": [
                {
                    ...
                    "mood": "tense",
                    "ambient_volume": 0.35,
                    "description": "Cursed forest manor"
                }
            ],
            "sfx_cues": []
        }

    bgm_raw = SOUNDSCAPES_DIR / "chapter_004_bgm.wav"

    seg_durations = {}
    for seg in script_data:
        s_idx = seg.get("index", 1)
        matches = sorted(AUDIO_CHUNKS_DIR.glob(f"c004_s{s_idx:04d}_*.wav"))
        if matches:
    ...
    cinematic_m4a = MASTERED_DIR / "chapter_004_cinematic.m4a"
    apply_dynamic_sidechain_ducking(
    ...
    logger.info("=" * 70)
    logger.info("🎉 CHAPTER 4 PRODUCTION COMPLETE!")
